...
/**
 * 📅 EVENTO DO CALENDÁRIO CRIADO/ATUALIZADO
 */
export async function notifyCalendarEvent(params: {
  userIds: string[];
  eventId: string;
  eventTitle: string;
  eventDate: Date;
  ...
  const { userIds, eventTitle, eventDate, eventType, createdByName, eventId, isUpdate } = params;
  ...
  const notification: PushNotificationData = {
    title: `${emoji} Evento ${action}: ${eventTitle}`,
    body: `${createdByName} | ${dateStr} | ${eventType}`,
    icon: '/icon-192x192.png',
    tag: `event-${eventId}`,
    data: {
      type: 'calendar_event',
  ...
  const notificationPromises = userIds.map(userId => 
    prisma.notification.create({
      data: {
        userId,
        title: `Evento ${action}: ${eventTitle}`,
        message: `${createdByName} ${action} evento para ${dateStr}`,
        type: 'CALENDAR_EVENT',
        relatedId: eventId
      }
    })
  ...
  // Enviar push para todos (verificação de preferências é feita no sendPushIfEnabled)
  // Como é evento de calendário, usaremos o tipo 'message' como fallback
