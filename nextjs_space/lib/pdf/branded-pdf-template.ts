...
<body>
  ...
  <!-- ATTENDANCE SUMMARY (if present) -->
  ${reportData.summary ? `
  <div class="section">
    <h2 class="section-title">${translations.overview || 'Resumo de Presenças'}</h2>
    <div class="stats-grid">
      <div class="stat-card">
        <div class="stat-label">${translations.attendanceRate || 'Taxa de Assiduidade'}</div>
        <div class="stat-value">${reportData.summary.attendanceRate || 0}%</div>
        <div class="stat-description">${reportData.summary.onTimeDays || 0} / ${reportData.summary.totalExpectedDays || 0} dias</div>
      </div>

      <div class="stat-card">
        <div class="stat-label">${translations.punctualityRate || 'Taxa de Pontualidade'}</div>
        <div class="stat-value">${reportData.summary.punctualityRate || 0}%</div>
        <div class="stat-description">${(reportData.summary.totalExpectedDays || 0) - (reportData.summary.lateDays || 0)} dias pontuais</div>
      </div>

      <div class="stat-card">
        <div class="stat-label">${translations.hoursWorked || 'Horas Trabalhadas'}</div>
        <div class="stat-value">${reportData.summary.totalHoursWorked || 0}h</div>
        <div class="stat-description">de ${reportData.summary.totalExpectedHours || 0}h previstas</div>
      </div>

      <div class="stat-card">
        <div class="stat-label">${translations.coverageRate || 'Taxa de Cobertura'}</div>
        <div class="stat-value">${reportData.summary.coverageRate || 0}%</div>
        <div class="stat-description">turnos realizados</div>
      </div>
    </div>
    ...
    <div style="margin-top: 30px;">
      <h3 style="font-size: 16px; font-weight: 600; margin-bottom: 15px; color: #374151;">
        ${translations.statusDistribution || 'Distribuição por Estado'}
      </h3>
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-label">${translations.present || 'Presente'}</div>
          <div class="stat-value" style="color: #10b981;">${reportData.summary.onTimeDays || 0}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">${translations.late || 'Atrasado'}</div>
          <div class="stat-value" style="color: #f59e0b;">${reportData.summary.lateDays || 0}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">${translations.absentJustified || 'Falta Justificada'}</div>
          <div class="stat-value" style="color: #6b7280;">${reportData.summary.absentJustifiedDays || 0}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">${translations.absentUnjustified || 'Falta Não Justificada'}</div>
          <div class="stat-value" style="color: #ef4444;">${reportData.summary.absentUnjustifiedDays || 0}</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <!-- USER STATS TABLE (if present) -->
  ${reportData.userStats && reportData.userStats.length > 0 ? `
  <div class="section">
    <h2 class="section-title">${translations.userStats || 'Detalhe por Utilizador'}</h2>
    <table>
      <thead>
        <tr>
          <th>${translations.name || 'Nome'}</th>
          <th class="text-center">${translations.expectedDays || 'Dias Previstos'}</th>
          <th class="text-center">${translations.workedDays || 'Dias Trabalhados'}</th>
          <th class="text-center">${translations.punctualDays || 'Dias Pontuais'}</th>
          <th class="text-center">${translations.daysWithDelay || 'Dias com Atraso'}</th>
          <th class="text-center">${translations.attendanceRate || 'Taxa de Assiduidade'}</th>
          <th class="text-center">${translations.punctualityRate || 'Taxa de Pontualidade'}</th>
          <th class="text-right">${translations.hoursWorked || 'Horas Trabalhadas'}</th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr>
          ...
          <td class="text-center">${user.expectedDays}</td>
          <td class="text-center">${user.workedDays}</td>
          <td class="text-center">${user.punctualDays}</td>
          <td class="text-center">${user.daysWithDelay}</td>
          <td class="text-center">
            <span class="badge ${user.attendanceRate >= 90 ? 'badge-success' : user.attendanceRate >= 70 ? 'badge-warning' : 'badge-info'}">
              ${user.attendanceRate}%
            </span>
          </td>
          <td class="text-center">
            <span class="badge ${user.punctualityRate >= 90 ? 'badge-success' : user.punctualityRate >= 70 ? 'badge-warning' : 'badge-info'}">
              ${user.punctualityRate}%
            </span>
          </td>
          <td class="text-right">${user.totalHours}h</td>
        </tr>
        `).join('')}
      </tbody>
    </table>
  ...
  <!-- DAILY BREAKDOWN (if present) -->
  ${reportData.dailyBreakdown && reportData.dailyBreakdown.length > 0 ? `
  <div class="section">
    <h2 class="section-title">${translations.dailyBreakdown || 'Evolução Diária'}</h2>
    <div class="chart-placeholder">
      📊 ${translations.chartInfo || 'Gráfico de evolução diária'}
      <br/>
      <small>${translations.chartNote || 'Dados disponíveis na versão interativa'}</small>
    </div>
    <table>
      <thead>
        <tr>
          <th>${translations.date || 'Data'}</th>
          <th class="text-center">${translations.present || 'Presentes'}</th>
          <th class="text-center">${translations.late || 'Atrasados'}</th>
          <th class="text-center">${translations.absent || 'Faltas'}</th>
        </tr>
      </thead>
      <tbody>
        ${reportData.dailyBreakdown.slice(0, 31).map(day => `
        <tr>
          <td>${new Date(day.date).toLocaleDateString('pt-PT', { day: '2-digit', month: '2-digit', year: 'numeric' })}</td>
          <td class="text-center"><span class="badge badge-success">${day.presentCount}</span></td>
          <td class="text-center"><span class="badge badge-warning">${day.lateCount}</span></td>
          <td class="text-center"><span class="badge badge-info">${day.absentCount}</span></td>
        </tr>
        `).join('')}
      </tbody>
    </table>
