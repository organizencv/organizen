...
export function ProfileContent({ user: initialUser }: ProfileContentProps) {
  ...
  const [emailSignature, setEmailSignature] = useState(initialUser?.emailSignature || '');
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <PageHeader title={getTranslation('profile', language)} />
        ...
        <Tabs defaultValue="personal" className="w-full">
          ...
          <TabsContent value="emergency">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  {language === 'pt' ? 'Assinatura de Email' : 'Email Signature'}
                </CardTitle>
                <CardDescription>
                  {language === 'pt' 
                    ? 'Assinatura adicionada automaticamente às mensagens' 
                    : 'Signature automatically added to messages'}
                </CardDescription>
              </CardHeader>
              <CardContent>
                {!isEditingSignature ? (
                  <div className="space-y-4">
                    ...
                    <Button onClick={() => setIsEditingSignature(true)}>
                      {emailSignature 
                        ? getTranslation('edit', language) 
                        : (language === 'pt' ? 'Adicionar Assinatura' : 'Add Signature')}
                    </Button>
                  </div>
                ) : (
