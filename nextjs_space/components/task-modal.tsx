  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requestData: any = {
        ...
        memberIds: selectedMembers, // Membros adicionais além do responsável principal
      };
          ...
          {availableUsers?.length > 1 && (
            <div>
              <Label className="flex items-center gap-2">
                ...
                {language === 'pt' ? 'Membros Adicionais' : 'Additional Members'}
              </Label>
              <p className="text-sm text-muted-foreground mb-2">
                {language === 'pt' 
                  ? 'Adicione outros utilizadores que colaborarão nesta tarefa'
                  : 'Add other users who will collaborate on this task'
                }
              </p>
              ...
              {/* Selector de membros disponíveis */}
              <Select
                value="none"
                onValueChange={(value) => {
                  if (value !== 'none' && !selectedMembers.includes(value) && value !== formData.userId) {
                    setSelectedMembers(prev => [...prev, value]);
                  }
                }}
              >
                <SelectTrigger>
                  <SelectValue placeholder={language === 'pt' ? 'Adicionar membro...' : 'Add member...'} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="none" disabled>
                    {language === 'pt' ? 'Selecionar utilizador' : 'Select user'}
                  </SelectItem>
                  {availableUsers
                    ?.filter(user => 
                      user?.id && 
                      user.id !== formData.userId && // Não mostrar o responsável principal
                      !selectedMembers.includes(user.id) // Não mostrar membros já selecionados
                    )
                    .map((user) => (
                      <SelectItem key={user.id} value={user.id}>
                        <div className="flex items-center gap-2">
