...
import { X, Calendar, User, Clock, AlertCircle, CheckSquare, MessageSquare, FileText, Tag, Paperclip, Users } from 'lucide-react';
...
export function TaskDetailsModal({
  ...
  }, [task?.id, task?.subtasks?.length, JSON.stringify(task?.subtasks)]);
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
          <TabsList className="flex-shrink-0 grid grid-cols-7 w-full">
            ...
            <TabsTrigger value="members" className="gap-1">
              ...
              <span className="hidden sm:inline">{getTranslation('taskMembers', language)}</span>
              {(taskData?.members?.length || 0) > 0 && (
                <Badge variant="secondary" className="ml-1">{taskData?.members?.length}</Badge>
              )}
            </TabsTrigger>
          ...
          <div className="flex-1 overflow-y-auto mt-4">
            ...
            <TabsContent value="members" className="mt-0">
              <TaskMembers
                ...
                task={taskData}
                users={users}
                canEdit={userRole === 'ADMIN' || userRole === 'MANAGER' || currentUserId === taskData.user.id}
                language={language}
                onUpdate={() => handleTaskUpdate(taskData)}
              />
            </TabsContent>

            <TabsContent value="subtasks" className="mt-0">
              <TaskSubtasks
                key={`subtasks-${taskData?.id}-${refreshKey}`}
                task={taskData}
                users={users}
                userRole={userRole}
                currentUserId={currentUserId}
