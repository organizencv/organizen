...
export function TasksContent({ tasks: initialTasks, users, userRole, currentUserId, openTaskId }: TasksContentProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: language === 'pt' ? 'Erro ao carregar tarefa' : 'Error loading task',
          variant: 'destructive'
        });
      }
    } catch (error) {
  ...
  const handleViewTask = async (task: Task) => {
    // Buscar tarefa completa com todos os dados (incluindo anexos)
  ...
  const handleTaskUpdated = (updatedTask: any) => {
    // Forçar nova referência do array para React detectar mudança
    setTasks(prevTasks => 
      prevTasks?.map(task => 
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <PageHeader title={getTranslation('tasks', language)}>
        {canCreateTasks && (
