...
import { ShiftAssignmentsManager } from './shift-assignments-manager';
import AttendanceContent from './attendance-content';
import { SortableList } from './ui/sortable-list';
import { Tabs, TabsContent, TabsList, TabsTrigger } from './ui/tabs';
import { getTranslation, Language } from '@/lib/i18n';
import { Plus, Search, Clock, Calendar, Edit, Trash2, User, Users, Eye, ClipboardCheck } from 'lucide-react';
...
export function ShiftsContent({ shifts: initialShifts, users, userRole, currentUserId, openShiftId }: ShiftsContentProps) {
  ...
  const [managingShift, setManagingShift] = useState<Shift | null>(null);
  const [viewingShift, setViewingShift] = useState<string | null>(null);
  ...
  const renderShift = (shift: Shift) => {
    ...
    return (
      <Card 
        ...
        onClick={() => setViewingShift(shift.id)}
      >
        <CardContent className="p-6">
          <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
            <div className="flex-1">
              ...
              {hasMultipleCapacity && assignedCount > 0 && (
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <PageHeader title={getTranslation('shifts', language)}>
        {canCreateShifts && (
      ...
      <Tabs defaultValue="shifts" className="space-y-6">
        ...
        {/* Tab: Registo de Ponto */}
        <TabsContent value="attendance" className="space-y-6">
          <AttendanceContent language={language} />
        </TabsContent>
      </Tabs>
      ...
      {/* Assignments Manager Modal */}
      {managingShift && (
        <ShiftAssignmentsManager
          shiftId={managingShift.id}
          shiftTitle={managingShift.title}
          shiftStartTime={managingShift.startTime}
          shiftEndTime={managingShift.endTime}
          capacity={managingShift.capacity || 1}
          currentAssignments={managingShift.assignments || []}
          onClose={() => setManagingShift(null)}
          onUpdated={() => {
            setManagingShift(null);
            fetchShifts();
          }}
          language={language}
        />
