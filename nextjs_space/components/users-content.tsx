...
export function UsersContent({ users: initialUsers, departments, teams, userRole }: UsersContentProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <PageHeader title={getTranslation('users', language)}>
      ...
      <Tabs value={viewMode} onValueChange={(value) => setViewMode(value as any)}>
        ...
        {/* By Department View */}
        <TabsContent value="byDepartment" className="mt-6">
          <Accordion type="multiple" className="space-y-4">
            {departments.map(dept => {
            ...
            {usersWithoutDepartment.length > 0 && (
              <AccordionItem value="no-department" className="border rounded-lg px-4">
                <AccordionTrigger className="hover:no-underline">
                  <div className="flex items-center gap-2">
